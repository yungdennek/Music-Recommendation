...
\begin{abstract}
This project presents a Music Recommendation System that combines content-based and collaborative filtering techniques to provide personalized music suggestions. We use a large-scale Kaggle listening-history dataset of roughly 500K users (top tracks/artists/albums with playcounts) as the primary source of implicit user-item interactions, and supplement it with a Kaggle ``Spotify Million Song'' lyrics dataset to enable content-based similarity based on song text. The system learns user preferences from past listening behavior and item characteristics to recommend new tracks that match a user's taste while still supporting discovery of unseen content. The architecture consists of modular components for data processing, content-based similarity, user-based collaborative filtering, and a hybrid layer that integrates both signals. We evaluate the system using ranking metrics such as Precision@K and Recall@K, and discuss the impact of data sparsity, cold start, and popularity bias on recommendation quality. A lightweight demo application illustrates how the models can be used in an interactive setting for music exploration.
\end{abstract}
...
\subsubsection{Music Listening Data (500K Users) (Primary)}

Our primary dataset is a large-scale Kaggle listening-history dataset with roughly 500K users. It provides user-level ``top'' lists (tracks, artists, albums) with associated playcounts, which we treat as implicit feedback for training and evaluating recommendation models.
...
\subsubsection{Spotify Million Song Dataset (Lyrics) (Supplementary)}

We additionally use a Kaggle dataset commonly referred to as the ``Spotify Million Song'' dataset, which contains track metadata and lyric text. This dataset is used to support content-based recommendation (e.g., TF-IDF over lyric text) and to make the demo outputs more interpretable.
...
In addition to this primary CF pipeline, we implemented a lightweight, heuristic user-based method for interactive demos: given a set of seed tracks, we first identify candidate users who listened to those tracks, then sample pairs of users with similar top-track profiles, and finally recommend tracks that are shared within those users' listening histories.
...
\begin{enumerate}
    ...
    \item \textbf{Dataset biases}: The datasets used are sourced from public listening histories and lyric corpora and may skew toward Western music and popular genres. Users with niche tastes may receive less relevant recommendations.
...
\begin{itemize}
    ...
    \item Pre-trained audio embeddings (when available from an external audio feature source)
...
\begin{enumerate}
    ...
    \item Spotify Million Song Dataset (Kaggle). \url{https://www.kaggle.com/datasets/notshrirang/spotify-million-song-dataset}
    
    \item Music Listening Data (500K Users) (Kaggle). \url{https://www.kaggle.com/datasets/gabrielkahen/music-listening-data-500k-users}
...
\textbf{Prerequisites:} Before running either setup, download the required datasets and place them in the \texttt{data/} directory. The top 50 songs/artists/albums datasets must be in the same directory as the notebook file for the demo to function correctly.
...
\subsection{Running the Jupyter Notebook Demo}

\begin{enumerate}
    \item Run all cells in the notebook sequentially.
    \item In the demo section (with the UI), you can enter a set of songs. Use ``\texttt{ | }'' after each song to specify a particular artist.
    \item Press ``Capture Songs'' then run the subsequent code blocks to receive recommended songs with YouTube video links.
    \item For artist and album recommendations, enter artists and/or albums as a list into the \texttt{artist\_recommender} and \texttt{album\_recommender} functions.
\end{enumerate}
